...
//        mPlayDevice.setGid("dev6");
//        mPlayDevice.setAccessPwd("88888888");
//        mPlayDevice.setAccessName("client002");
    ...
    private void initMatrix(Matrix matrix, int videoWidth, int videoHeight) {
        ...
        int winWidth = videowindow.getWidth();
        int winHeight = videowindow.getHeight();
...
//            matrix.postTranslate(translateX, translateY);
            ...
            float scaleX = winWidth / (videoWidth * 1.0f);
            float scaleY = winHeight / (videoHeight * 1.0f);

            matrix.postScale(scaleX,scaleY);
            matrix.postTranslate( (winWidth - videoWidth * scaleX) / 2f,
                    (winHeight - videoHeight * scaleY) / 2f);
        }
    }
